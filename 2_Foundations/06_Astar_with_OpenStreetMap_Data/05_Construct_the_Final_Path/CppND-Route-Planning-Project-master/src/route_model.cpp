...
**
這邊要兩次 RouteModel::Node, 最前面的 RouteModel::Node* 是return type,
接下了來的是指你要implement 的 FindNeightbor 是 RouteModel::Node 下的function

** 因此 functioin 內的 this 指的是 Node, 這次不是 RouteModel
*/
RouteModel::Node* RouteModel::Node::FindNeighbor(std::vector<int> node_indices){
    ...
    std::vector<RouteModel::Node>& routemodel_m_Nodes = this->parent_model->SNodes();
    ...
    for (int node_idx : node_indices){
        ...
        //我們要的Node 是又包好gh 值的 RouteModel node, 從 this->SNodes()拿
        RouteModel::Node& cur_other_node = routemodel_m_Nodes[node_idx];
        //For each retrieved Node in the loop, you should check that the node has not been
        //visted (!node.visited) and that the distance to this is nonzero.
        //In other words, you want the closest unvisted node that is not the current node.
        float distance = this->distance(cur_other_node);


        //my way
        if (cur_other_node.visited == false and distance!=0.0){
            if (distance < mind){
                mind = distance;
                closest_node = &cur_other_node;
            }
        }

        //method2
        //if(closest_node == nullptr || this->distance(cur_other_node) < this->distance(closest_node)){
        //   closest_node = &cur_other_node;
        //}
...
void RouteModel::Node::FindNeighbors(){
    ...
    int curNodeidx = this->index;
    //mapping the roadS
    std::vector<const Model::Road*> roads_belong = this->parent_model->node_to_road[curNodeidx];
    //get all ways
    const std::vector<Way>& cur_m_ways = this->parent_model->Ways();
    //you can always get the reference by add &
    for(const Model::Road* &road : roads_belong){

        //because cur_m_ways is type const above, so we can't change the
        //way_node_indices to & way_node_indices, need follow the original
        //design in Way object, which the nodes is std::vector<int>
        std::vector<int>  way_node_indices = cur_m_ways[road->way].nodes;
        RouteModel::Node* clostNode_to_this_road = this->FindNeighbor(way_node_indices);
        if (clostNode_to_this_road !=nullptr)// FindNeighbor 可能都找不到, return null, 要檢查
            this -> neighbors.push_back(clostNode_to_this_road);
    }
...
/*
//用來找一個離使用者輸入x,y 座標最近的 Node, 畢竟要能對應上open street map 才能繼續下去
...
RouteModel::Node& RouteModel::FindClosestNode(float x,float y){
    ...
    //loop all roads, 從所有的roads 找起
    const std::vector<Road>& all_m_roads = this->Roads();

    for (const Model::Road &road : all_m_roads){
        if(road.type != Model::Road::Type::Footway){
            for(int node_idx : this->Ways()[road.way].nodes){

                RouteModel::Node & other = this->SNodes()[node_idx];
                float dis = inputNode.distance(other);
                if (dis < min_dist){
                    min_dist = dis;

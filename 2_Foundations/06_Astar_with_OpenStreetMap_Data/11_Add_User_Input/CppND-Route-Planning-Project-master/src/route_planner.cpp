...
RouteModel::Node* RoutePlanner::NextNode(){
    ...
    RouteModel::Node * minfNode = open_node_list.front();

    //因為 earse 需要一個 iterator, 因此放 .begin
    open_node_list.erase(open_node_list.begin());
    return minfNode;
}
...
void RoutePlanner::AStarSearch(){
    ...
    //initial the start_node as the first open node list
    this->open_node_list.push_back(this->start_node);

    RouteModel::Node *current_node = nullptr;

    //有點BFS的味道
    while(open_node_list.size()>0){

        current_node = this->NextNode();

        //current_node is the end_node: Finished
        if(current_node->distance(*(this->end_node)) == 0){
            m_Model.path = this->ConstructFinalPath(current_node);
            return;
        }else{
        //still not at end_node, keep expanding open node list
        // AddNeighors() will help us add the clostest node into open node list

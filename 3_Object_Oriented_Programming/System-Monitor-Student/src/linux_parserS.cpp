...
float LinuxParserS::MemoryUtilization() { 
  string line;
  string key;
  long memValue;
  long memTotal = 0;
...
long LinuxParserS::ActiveJiffies(int pid) {
  ...
  LinuxParserS::CpuProcessInfo LinuxParserS::GetProcessCpuInfo(int pid) {
    std::stringstream filename;
    filename << kProcDirectory << "/" << pid << "/" << kStatFilename;
    std::ifstream filestream(filename.str());
    LinuxParserS::CpuProcessInfo returnInfo;
    if (filestream.is_open()) {
        std::string line;
        std::getline(filestream, line);
        std::istringstream linestream(line);
        ...
        for(int i = 0; i < 4; i++) linestream >> ignore;
        linestream >> starttime;
        returnInfo.seconds = LinuxParserS::UpTime() - (starttime/sysconf(_SC_CLK_TCK));
        returnInfo.totalTime = (utime + stime + cutime + cstime)/sysconf(_SC_CLK_TCK);
    }   
    return returnInfo;
  }

  float LinuxParserS::CpuUtilization(int pid)
  {
    //LinuxParserS::CpuProcessInfo previous = LinuxParserS::GetProcessCpuInfo(pid);
    //sleep(1);
    LinuxParserS::CpuProcessInfo current = LinuxParserS::GetProcessCpuInfo(pid);
...
long LinuxParserS::ActiveJiffies() {
  std::ifstream filestream(kProcDirectory + kUptimeFilename);
  long upTime = 0;
  long idleTime;
  if (filestream.is_open()) {
...
vector<LinuxParserS::CpuKPI> LinuxParserS::CpuUtilPercentage() {
  std::ifstream filestream(kProcDirectory + kStatFilename);
  vector<LinuxParserS::CpuKPI> returnVector;
  if (filestream.is_open()) {
      std::string line;
      while (std::getline(filestream, line)) {
          std::istringstream linestream(line);
...
vector<string> LinuxParserS::CpuUtilization() { 
  std::vector<LinuxParserS::CpuKPI> previousVector = LinuxParserS::CpuUtilPercentage(); 
  sleep(1);
...
int LinuxParserS::TotalProcesses() { 
  std::ifstream filestream(kProcDirectory + kStatFilename);
...
int LinuxParserS::RunningProcesses() { 
  std::ifstream filestream(kProcDirectory + kStatFilename);
...
// REMOVE: [[maybe_unused]] once you define the function
string LinuxParserS::Command(int pid) { 
 std::stringstream filename;
...
// REMOVE: [[maybe_unused]] once you define the function
string LinuxParserS::Ram(int pid) { 
  std::stringstream filename;
...
// REMOVE: [[maybe_unused]] once you define the function
string LinuxParserS::Uid(int pid) { 
  std::stringstream filename;
  filename << kProcDirectory << "/" << pid << "/" << kStatusFilename;
  std::ifstream filestream(filename.str());
  string uid ;
...
// REMOVE: [[maybe_unused]] once you define the function
string LinuxParserS::User(int pid) { 
...
// REMOVE: [[maybe_unused]] once you define the function
long LinuxParserS::UpTime(int pid) { 
  std::stringstream filename;
  filename << kProcDirectory << "/" << pid << "/" << kStatFilename;
  std::ifstream filestream(filename.str());
  long starttime = 0;
  if (filestream.is_open()) {
      ...
      std::time_t elapsedTime = LinuxParserS::UpTime() - (starttime/sysconf(_SC_CLK_TCK));
      return elapsedTime;
  }
  return starttime; 
  }